    ...
    for (ABlock* B : Blocks)
    {
        ...
        if (b)
        {
            for (auto&& Result : OutOverlaps)
            {
                ...
                if (Result.GetActor()->GetName() == "Cube_4") {
                    UE_LOG(LogTemp, Warning, TEXT("Nueva Puntuacion"))
                        ScorePuntos =ScorePuntos+ 10;
                    UE_LOG(LogTemp, Warning, TEXT("Los Score De Puntos Es: %d"),ScorePuntos)
                }
            }
            return true;
        }
